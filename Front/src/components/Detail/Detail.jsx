...
function Detail() {
  ...
  useEffect(() => {
    ...
    axios(`http://localhost:3001/rickandmorty/detail/${detailId}?key=${KEY}`).then((response) =>
      ...
      {character.name ? (
        <div className={style.containerCard}>
          <div className={style.divFlex}>
            ...
            <br />
            <p className={style.textCard}>Location: {character.location}</p>
            <br />
            <p className={style.textCard}>Origin: {character.origin}</p>
          </div>
